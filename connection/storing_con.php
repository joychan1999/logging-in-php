...
class Databases
{
    ...
    //PLACING ORDER
    public function placeOrder($orderID, $userID, $sellerID, $productName, $productImage, $productQuantity, $productPrice, $subtotal,$status)
    {
        $sql = "INSERT INTO orders(orderID,buyerID,sellerID,productName, productImage,productQuantity,productPrice,subtotal,status) VALUES('" . $orderID . "','" . $userID . "','".$sellerID."','" . $productName . "','" . $productImage . "','" . $productQuantity . "','" . $productPrice . "','" . $subtotal . "','".$status."')";
        $query = $this->con->query($sql);
        if ($query) {
            return $query;
        } else {
